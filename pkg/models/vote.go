...
func debug(text, line string) {
	fmt.Printf("%s -- on line %s", text, line)
}
...
func (v *Vote) DoTheThing() error {
	...
	if err := votes_collection.FindOne(ctx, filter).Decode(&xvote); err != nil {
		...
		if v.IsUpvote == xvote.IsUpvote {
			...
			update := bson.M{"$set": bson.M{"is_upvote": !xvote.IsUpvote, "updated_at": time.Now()}}
			filter := bson.M{"_id": xvote.ID}
